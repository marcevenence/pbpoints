...
class PlayerResourceIT {
    ...
    @Test
    @Transactional
    void getAllPlayersByCategoryIsEqualToSomething() throws Exception {
        // Initialize the database
        playerRepository.saveAndFlush(player);
        ...
        // Get all the playerList where category equals to categoryId
        defaultPlayerShouldBeFound("categoryId.equals=" + categoryId);

        // Get all the playerList where category equals to (categoryId + 1)
        defaultPlayerShouldNotBeFound("categoryId.equals=" + (categoryId + 1));
    }
